...
func (suite *TypesTestSuite) TestVerifyMisbehaviourTendermint() {
	...
	// altVal := comettypes.NewValidator(altPubKey, 100)

	// Create alternative validator set with only altVal, invalid update (too much change in valSet)
	// altValSet := comettypes.NewValidatorSet([]*comettypes.Validator{altVal})
	...
	testCases := []struct {
	...
	}{
		{
			"valid fork misbehaviour", func() {
				...
				cometMisbehaviour := &ibctm.Misbehaviour{
				...
				wasmData, err := suite.chainB.Codec.MarshalInterface(cometMisbehaviour)
				suite.Require().NoError(err)
				clientMessage = &types.ClientMessage{
					Data: wasmData,
				}
		...
		// 		bothValSet := comettypes.NewValidatorSet(append(suite.chainB.Vals.Validators, altValSet.Proposer))
		...
		// 		clientMessage = &solomachine.Misbehaviour{}
		// 	}, false,
		// },
		// {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			fmt.Println("clientState", clientState.ClientType())
			clientStore := suite.chainA.App.GetIBCKeeper().ClientKeeper.ClientStore(suite.chainA.GetContext(), path.EndpointA.ClientID)
			err := clientState.VerifyClientMessage(suite.chainA.GetContext(), suite.chainA.App.AppCodec(), clientStore, clientMessage)
